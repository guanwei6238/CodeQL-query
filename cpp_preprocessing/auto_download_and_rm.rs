...
//! serde_json = "1.0.148"
//! ```
/// 使用實際的將 cpp_autoql.sh 的掃描部分 (最後for 迴圈) 註解掉。
use clap::Parser;
use csv::Reader;
...
struct Args {
    ...
    /// csv path
    #[arg(long, default_value = "../repos/repos_new_cpp.csv")]
    csv: String,
...
fn main() {
    ...
    let time_cache_path = "./cpp_time_cache.txt";
    ...
    for res in rdr.deserialize::<Row>() {
        ...
        let json_path = format!("./cpp_query_output/{}/{}.json", &repo_name, &repo_name);
        ...
        if !file_paths.is_empty() {
            ...
            let secs: u64 = match time_cache.get(repo_name).copied() {
                ...
                None => {
                    ...
                    let st = Command::new("./cpp_autoql.sh")
